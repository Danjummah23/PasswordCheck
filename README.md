...
>
> A **Password Vulnerability Detector** that checks your password against a comprehensive set of security rules and even queries the real-world [Have I Been Pwned](https://haveibeenpwned.com/) breach database.
...
PasswordCheck analyses a password through a pipeline of checks, computes its entropy, estimates crack time, and queries the HIBP API to see if it has appeared in known data breaches. At the end it generates a security score and optionally suggests a stronger password.

The application has three layers:

1. **`password_check.py`** — The core Python engine that runs all checks and produces a report.
2. **`api.py`** — A lightweight Flask REST API that exposes the engine over HTTP so any client can use it.
3. **`frontend/`** — A React + Vite web interface that talks to the API so you can check passwords in your browser without touching a terminal.
...
4. Type a password into the input field and get your results instantly — no terminal required.
...
|---|---|---|
| **Length** | Passwords under 8 chars are rejected; under 12 is a warning | Critical / Warning |
| **Character variety** | Requires a mix of uppercase, lowercase, digits, and special chars | Critical / Warning |
| **Keyboard walk** | Detects sequences like `qwerty`, `asdfgh`, `12345` — yes, typing across the keyboard counts as creativity to exactly no one | Critical |
| **Repeated characters** | Flags patterns like `aaa` or `111` | Warning |
| **Sequential numbers** | Catches runs like `123`, `456` | Warning |
| **Date patterns** | Spots birth years, MMDD, DDMM, YYMMDD — because your birthday is not a secret | Warning |
| **Leet speak** | Sees through `p@ssw0rd` and `l3tm31n` — the hacker aesthetic that fools no one | Critical |
| **Dictionary words** | Matches against 150+ common words including names, brands, pop culture, and sports | Warning |
| **Personal info** | Checks if your name, username, or birth year appears in the password | Critical |
| **HIBP breach check** | Queries the Have I Been Pwned API (k-anonymity model — only the first 5 chars of the SHA-1 hash are sent) | Critical |

### How the HIBP check protects your privacy

Your actual password is **never sent** to any external server. The tool computes the SHA-1 hash of your password, sends only the first 5 characters of that hash to the HIBP API, and compares the response to check if your full password appears in any known breach. This is the k-anonymity model and it's been validated by security researchers.

---

## Scoring System

The final score is determined by the number of critical and warning issues found:
...
If any issues are found, the tool also generates a **strong password suggestion** (18 characters, mixed case, digits and symbols) using Python's `secrets` module — because at least one of us needs to practice basic security hygiene.
...
| Flag | Description |
|---|---|
| `--no-hibp` | Skip the breach check (offline mode) |
...
>
> PasswordCheck est un **détecteur de vulnérabilité de mot de passe** qui analyse votre mot de passe à travers une série de vérifications de sécurité et consulte la base de données réelle [Have I Been Pwned](https://haveibeenpwned.com/).
...
PasswordCheck analyse un mot de passe à travers un ensemble de vérifications, calcule son entropie, estime le temps nécessaire pour le craquer, et interroge l'API HIBP pour vérifier s'il a été compromis. À la fin, il génère un score de sécurité et propose éventuellement un mot de passe plus robuste.
...
3. **`frontend/`** — Une interface web React + Vite qui communique avec l'API pour vous permettre de vérifier vos mots de passe depuis un navigateur, sans toucher à un terminal.
...
4. Saisissez un mot de passe dans le champ de saisie et obtenez vos résultats instantanément — sans terminal.
...
|---|---|---|
| **Longueur** | Moins de 8 caractères est refusé ; moins de 12 est un avertissement | Critique / Avertissement |
...
| **Séquence clavier** | Détecte des séquences comme `qwerty`, `asdfgh`, `12345` — car taper en ligne sur le clavier n'est pas de la créativité | Critique |
...
| **Patterns de date** | Repère les années de naissance, formats MMJJ, JJMM, AAMMJJ — car votre date de naissance n'est pas un secret | Avertissement |
| **Leet speak** | Voit à travers `p@ssw0rd` et `l3tm31n` — l'esthétique hacker qui ne trompe personne | Critique |
...
Votre mot de passe réel n'est **jamais envoyé** à un serveur externe. L'outil calcule le hash SHA-1 de votre mot de passe, envoie uniquement les 5 premiers caractères de ce hash à l'API HIBP, et compare la réponse pour vérifier si votre mot de passe complet apparaît dans une fuite de données. C'est le modèle de k-anonymité et il a été validé par des chercheurs en sécurité.

---

## Système de score

Le score final est déterminé par le nombre de problèmes critiques et d'avertissements détectés :
...
En cas de problème détecté, l'outil génère également une **suggestion de mot de passe robuste** (18 caractères, majuscules/minuscules, chiffres et symboles) via le module Python `secrets` — parce qu'au moins l'un de nous doit pratiquer l'hygiène de sécurité basique.
...
| Option | Description |
|---|---|
| `--no-hibp` | Ignorer la vérification de fuite (mode hors ligne) |
...
*Guide généré avec ❤️ — et un soupçon de honte pour chaque "password123" encore en circulation.*
